...
最近おいしい健康Androidチーム内でテスト推進に励んでいますkako351です。今まではもっぱらUnitテストを書いてきましたが、最近はUIテストにも関心が出てきました。
この章では、Jetpack ComposeでUIテストを実装する方法を説明します。

== UIテストとは

UIテストは、ユーザーインターフェース（UI）を対象とした自動化テストのことです。UIテストは、ソフトウェアのUIが意図した通りに機能し、ユーザーが期待する動作をするかどうかを確認できます。
Jetpack composeで実装されたUIをテストする場合、UIテストを実行するためのAPIが提供されておりUIテストを簡単に実装できます。
